...
app.add_middleware(
    ...
    allow_origin_regex=r"https://(.*\.)?vk\.com|https://(.*\.)?vk-apps\.com|https://(.*\.)?userapi\.com",
    ...
    with get_db() as conn:
        ...
        if existing:
            ...
            if cloud_user:
                ...
                rc = cloud_user.get("referral_code") or "".join(
                ...
                cursor.execute(
                    ...
                    (
                        ...
                        photo,
                        rc,
                        float(cloud_user.get("balance", 0)),
                        int(cloud_user.get("credits", 3)),
                        uid,
                    ),
                )
                conn.commit()
...
def verify_vk_signature(params: dict) -> bool:
    ...
    if not VK_MINIAPP_SECRET:
        print("[VK_MINIAPP] Секретный ключ не задан")
        return False
    ...
    hash_value = hmac.new(
        VK_MINIAPP_SECRET.encode("utf-8"),
        query_string.encode("utf-8"),
        hashlib.sha256,
    ).digest()

    calculated_sign = base64.urlsafe_b64encode(hash_value).decode("utf-8").rstrip("=")
...
async def homepage(request: Request):
    ...
    </head><body><div><h1>🚀 Backend работает!</h1>
    <p>AI Assistant Pro API v3.0</p>
    <p><a href="/health" style="color:#ffd700">Health Check</a> | <a href="{FRONTEND_URL}" style="color:#ffd700">Фронтенд</a></p>
    </div></body></html>
    """
...
async def health():
    ...
    miniapp = "configured" if VK_MINIAPP_SECRET else "not_configured"
    return {
        "status": "ok",
        "version": "3.0.0",
        "gigachat": gc,
...
async def vk_login_url(ref: str = None):
    ...
    login_url = (
        ...
        f"&code_challenge={code_challenge}&code_challenge_method=S256&scope=vkid.personal_info"
    )
...
async def process_vk_auth(code: str, device_id: str = "", state: str = ""):
    ...
    try:
        ...
        if "access_token" not in token_data:
            return HTMLResponse(
                content=f"<html><body><h1>Ошибка</h1><p>{json.dumps(token_data, ensure_ascii=False)}</p></body></html>"
            )
        ...
        return HTMLResponse(
            content=f"<html><head><meta http-equiv='refresh' content='0;url={url}'></head><body><script>window.location.href='{url}';</script></body></html>"
        )
    except Exception as e:
        return HTMLResponse(content=f"<html><body><h1>Ошибка</h1><p>{e}</p></body></html>")


@app.post("/auth/vk-miniapp")
async def vk_miniapp_auth(data: dict):
    ...
    if not vk_user_id:
        raise HTTPException(status_code=400, detail="Нет vk_user_id")
    ...
    if VK_SERVICE_TOKEN:
        try:
            ...
            if result:
                u = result[0]
                name = f"{u.get('first_name','')} {u.get('last_name','')}".strip() or name
...
async def create_payment(payment_data: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        r = requests.post(
            ...
            json=payload,
            headers={"Authorization": f"Basic {auth_b64}", "Content-Type": "application/json", "Idempotence-Key": str(uuid.uuid4())},
            timeout=20,
        )
...
if __name__ == "__main__":
    print("=" * 60)
    print("AI ASSISTANT PRO v3.0 - VK Mini App Ready")
    print("=" * 60)
    create_tables()
